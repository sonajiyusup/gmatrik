<?php 
	...
	function shalatWktByPeriodPercent($idPeriod1, $idPeriod2, $wkt){
		$ambildata = mysql_query("SELECT a.jml AS a, b.jml AS b, ROUND((((b.jml-a.jml)/a.jml)*100),2) AS 'percent' FROM ( SELECT s.id_periode, COUNT(s.wkt_tapping) AS jml FROM shalat s WHERE s.id_periode = $idPeriod1 AND s.wkt_shalat = '$wkt' GROUP BY s.id_periode ) a JOIN ( SELECT s.id_periode, COUNT(s.wkt_tapping) AS jml FROM shalat s WHERE s.id_periode = $idPeriod2 AND s.wkt_shalat = '$wkt' GROUP BY s.id_periode ) b") or die(mysql_error());
