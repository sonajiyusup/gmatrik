                          ...
                          function getChartJs(type) {
                              ...
                              if (type === 'bar') {
                                  config = {
                                      ...
                                      data: {
                                          labels: [<?php
                                                    $data = tahsinByTahsinRolePembina($idPembina);
                                                    foreach ($data as $row){
                                                     echo '"'.$row['tahsin'].'",';
                                                    }
                                                  ?>],
                                          datasets: [{
                                              label: "Nilai",
                                              data: [<?php
                                                    $dataNilai = tahsinByTahsinRolePembina($idPembina);
                          ...
                          <div class="table-responsive">
                            <table id="tableTahsinMhs" class="table table-hover table-condensed">
                              ...
                              <tbody>
                                <?php 
                                  $no = 1;
                                  $dataPresensi = tahsinByTahsinRolePembina($idPembina);
                                  foreach($dataPresensi as $row){
                                 ?>
                                <tr>
                                  <td><?php echo $no; ?></td>
                                  <td><?php echo '<a href="?page=tahsintdetail&w='.$row['tahsin'].'">'.ucfirst($row['tahsin']).'</a>' ?></td>
